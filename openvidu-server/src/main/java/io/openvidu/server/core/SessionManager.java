...
public abstract class SessionManager {
    ...
    //modify by jeffrey for newParticipant with  participantPublicId
    public Participant newParticipant(String sessionId, String participantPrivatetId, String participantPublicId,
                                      Token token, String clientMetadata, GeoLocation location, String platform,
                                      String finalUserId) {
        if (this.sessionidParticipantpublicidParticipant.get(sessionId) != null) {
            //String participantPublicId = RandomStringGenerator.generateRandomChain();
            Participant p = new Participant(finalUserId, participantPrivatetId,
                    participantPublicId, sessionId, token,
                    clientMetadata, location, platform, null);
            this.sessionidParticipantpublicidParticipant.get(sessionId)
                    .putIfAbsent(participantPublicId, p);
            //modify by jeffrey
    ...
    public Participant newCallParticipant(String sessionId,
        ...
        Token tokenObj = tokenGenerator.generateToken(
                sessionId, OpenViduRole.MODERATOR, "", new KurentoTokenOptions(new JsonObject()));

        if (this.sessionidParticipantpublicidParticipant.get(sessionId) != null) {
            Participant p = new Participant(null, participantPrivatetId, participantPublicId,
